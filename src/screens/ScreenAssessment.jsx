  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 18, padding: 16, marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.1em" }}>{t(m.name).toUpperCase()}</Mono>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 2 }}>
            <span style={{ fontFamily: C.display, fontWeight: 800, fontSize: 27, color: col, letterSpacing: "-0.02em" }}>{m.mine}</span>
            <span style={{ fontFamily: C.mono, fontSize: 11, color: C.muted }}>{m.unit}</span>
          </div>
        </div>
        <span style={{ padding: "4px 10px", borderRadius: 999, background: `${col}1f`, border: `1px solid ${col}55` }}>
          <Mono style={{ color: col, fontSize: 9 }}>{statusLabel(s, t)}</Mono>
        </span>
      </div>
        ...
        <div style={{ display: "flex", gap: 14 }}>
          {items.map(it => (
            <div key={it.label} style={{ textAlign: "center" }}>
              ...
              <div style={{ fontFamily: C.display, fontWeight: 700, fontSize: 13.5, color: it.col }}>{it.val}<span style={{ fontSize: 10, color: C.muted }}>{m.unit}</span></div>
            </div>
          ))}
        </div>
        <span style={{ color: C.muted, fontSize: 13.5, transition: "transform 0.2s", transform: expanded ? "rotate(90deg)" : "rotate(0deg)", alignSelf: "center" }}>›</span>
      </button>
  ...
  return (
    <div style={{ padding: "10px 18px 28px", color: C.text }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, padding: 18, marginBottom: 18, display: "flex", alignItems: "center", gap: 18 }}>
        ...
        <div style={{ flex: 1 }}>
          <Mono style={{ color: C.muted, fontSize: 10 }}>SKUPNA OCENA PERFORMANSA</Mono>
          <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 24.5, color: C.text, marginTop: 4 }}>
            {overall}<span style={{ fontSize: 13.5, color: C.muted, fontWeight: 600 }}>/100</span>
          </div>
          <div style={{ display: "flex", gap: 6, marginTop: 8, flexWrap: "wrap" }}>
            ...
            <Mono style={{ color: C.muted, fontSize: 9, marginLeft: 4 }}>{t("5 kazalnikov")}</Mono>
          </div>
        </div>
        <div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, padding: 18, marginBottom: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          ...
          <div>
            <div style={{ fontFamily: C.display, fontWeight: 700, fontSize: 15.5, color: C.text }}>{COACH_COMMENT.coach}</div>
            <Mono style={{ color: C.muted, fontSize: 9 }}>{COACH_COMMENT.date}</Mono>
          </div>
          <span style={{ marginLeft: "auto", padding: "3px 8px", borderRadius: 999, background: `${C.accent}1f`, border: `1px solid ${C.accent}40` }}>
            <Mono style={{ color: C.accent, fontSize: 9 }}>{COACH_COMMENT.grade}</Mono>
          </span>
        </div>

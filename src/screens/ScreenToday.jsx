...
function LiquidMetric({ value, max, label, color, decimals = 0, fillAlpha = 0.62, C, size = 122 }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 11 }}>
      ...
      <span style={{ fontFamily: C.mono, fontSize: 9, fontWeight: 600, letterSpacing: "0.22em", color: dark ? "rgba(255,255,255,0.55)" : C.muted, textTransform: "uppercase" }}>{label}</span>
    </div>
  );
}
...
function QuickAddSheet({ C, t, onClose, onSave }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 30, background: "rgba(0,0,0,0.55)" }} onClick={onClose}>
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: C.bg, borderRadius: "28px 28px 0 0", padding: "0 13px 20px", maxHeight: "88vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        {step === 0 ? (
          <>
            ...
            {[
              ...
              <button key={i} onClick={() => isActive && setStep(1)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 11px", marginBottom: 8, background: C.surface, border: `1px solid ${isActive ? C.accent : C.border}`, borderRadius: 14, cursor: isActive ? "pointer" : "default", textAlign: "left", opacity: isActive ? 1 : 0.42, WebkitTapHighlightColor: "transparent" }}>
                <span style={{ width: 34, height: 34, borderRadius: 9, background: `${isActive ? C.accent : C.muted}18`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{icon}</span>
                <span style={{ flex: 1 }}>
          ...
          <>
            ...
            <button onClick={() => { if (!valid) return; const partLabel = form.part === "Drugo" ? form.customPart.trim() : t(form.part); onSave({ name: `${t("Poškodba")} · ${partLabel}`, grade: form.grade, phase: 0, progressNote: form.note || t("Sveža poškodba — začetek protokola."), returnWeeks: form.grade * 2, returnDate: `${t("za")} ${form.grade * 2} ${t("tedna")}`, coachNote: "" }); onClose(); }} style={{ width: "100%", padding: "11px", borderRadius: 999, border: "none", background: valid ? C.btn : C.surface3, color: valid ? C.btnText : C.muted, fontFamily: C.display, fontWeight: 800, fontSize: 14, cursor: valid ? "pointer" : "default", letterSpacing: "0.04em", WebkitTapHighlightColor: "transparent" }}>{t("SHRANI POŠKODBO")}</button>
          </>
        )}
      </div>
    </div>
      ...
      <div style={{ marginBottom: 20, ...rise(0.15) }}>
        ...
        <Card>
          {recentCheckins.length === 0 ? (
            <div style={{ textAlign: "center", padding: "5px 0", color: C.muted, fontFamily: C.display, fontStyle: "italic", fontSize: 13.5 }}>
            ...
            return (
              <div key={r.date} style={{ padding: "10px 0", borderTop: idx ? `1px solid ${C.border}` : "none" }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 9 }}>
                  <span style={{ fontFamily: C.display, fontWeight: 700, fontSize: 13.5, color: C.text }}>{dLabel}</span>
                  {isToday && <Mono style={{ fontSize: 8, color: C.accent }}>{t("DANES")}</Mono>}
                </div>

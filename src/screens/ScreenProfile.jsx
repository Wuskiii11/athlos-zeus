...
import { Pressable, PrimaryBtn, BackBtn, LanguageSwitcher } from "../components/UI";
...
export default function ScreenProfile({ go, profile, setProfile }) {
  ...
  return (
    <div style={{ padding: "10px 18px 28px", color: C.text }}>
      ...
      <span style={{ ...label, marginTop: 22 }}>{t("JEZIK")}</span>
      <LanguageSwitcher value={curLang} onChange={setLang} style={{ marginTop: 8 }} />

      <PrimaryBtn onClick={save} style={{ marginTop: 30 }}>{t("Shrani profil")}</PrimaryBtn>
    </div>
  );
}

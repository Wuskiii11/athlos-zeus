...
export default function WelcomeTour({ onDone }) {
  ...
  return (
    <div style={{
      ...
      <button
        onClick={onDone}
        style={{ alignSelf: "flex-end", background: "none", border: "none", color: C.muted, fontFamily: C.display, fontSize: 13.5, fontWeight: 600, cursor: "pointer", padding: 6 }}
      >
        Skip
      </button>
      ...
      <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", animation: "athlosFade 0.35s ease" }}>
        <div style={{
          ...
          marginBottom: 28, boxShadow: C.glowSoft,
        }}>
          <Icon name={s.icon} color={C.accent} size={40} />
        </div>
        <h1 style={{ fontFamily: C.display, fontWeight: 800, fontSize: 26, margin: "0 0 12px", letterSpacing: "-0.02em" }}>{s.title}</h1>
        <p style={{ fontFamily: C.display, fontSize: 15, lineHeight: 1.6, color: C.text2, margin: 0, maxWidth: 320 }}>{s.desc}</p>
      </div>

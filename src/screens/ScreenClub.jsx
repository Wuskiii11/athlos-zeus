...
function BatteryBar({ pct, C }) {
  ...
  return (
    <div style={{ height: 4, borderRadius: 999, background: C.surface3, overflow: "hidden", marginTop: 8 }}>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 20, background: C.bg, display: "flex", flexDirection: "column", animation: "athlosFade 0.2s ease", overflowY: "auto" }}>
      ...
      <div style={{ padding: "12px 18px 14px", display: "flex", alignItems: "center", gap: 12, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
        <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: "50%", border: "none", background: C.surface2, color: C.text, fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, WebkitTapHighlightColor: "transparent" }}>←</button>
        <span style={{ width: 42, height: 42, borderRadius: "50%", background: C.surface3, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: C.display, fontWeight: 700, color: C.text2, fontSize: 15, flexShrink: 0 }}>{athlete.ini}</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 19, color: C.text, letterSpacing: "-0.01em" }}>{athlete.name}</div>
          <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.08em" }}>{t(athlete.pos)}</Mono>
        </div>
        <button onClick={() => { onClose(); go("chat"); }} aria-label="Chat" style={{ padding: "10px 14px", borderRadius: 999, border: "none", background: C.surface2, color: C.text2, cursor: "pointer", WebkitTapHighlightColor: "transparent", display: "flex", alignItems: "center" }}><IcChat size={15} /></button>
      </div>

      <div style={{ flex: 1, padding: "24px 18px 32px" }}>
        {/* battery — dominant element */}
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase" }}>{t("READINESS · BATERIJA")}</Mono>
            <Mono style={{ color: batCol, fontSize: 10, letterSpacing: "0.14em" }}>{athlete.battery >= 70 ? "READY" : athlete.battery >= 40 ? "CAUTION" : "REST"}</Mono>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
            ...
            <span style={{ fontFamily: C.mono, fontSize: 13, color: C.muted }}>/100</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
          {[["Sprint", athlete.sprint], ["Počep 1RM", athlete.squat]].map(([l, v]) => (
            <div key={l} style={{ flex: 1, background: C.surface2, borderRadius: 20, padding: "14px 16px" }}>
              <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", display: "block" }}>{t(l.toUpperCase())}</Mono>
              <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 22, color: C.text, marginTop: 6, letterSpacing: "-0.01em" }}>{v}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase" }}>{t("WELLNESS CHECK-IN")}</Mono>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: C.display, fontWeight: 800, fontSize: 17, color: athlete.streak > 0 ? C.text : C.muted2 }}>
              ...
              {athlete.streak} <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.08em" }}>{t("DNI ZAPORED")}</Mono>
            </span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {[["Spanje", athlete.wellness.sleepQuality, false], ["Bolečine", athlete.wellness.soreness, true], ["Stres", athlete.wellness.stress, true], ["Energija", athlete.wellness.mood, false]].map(([l, v, badHigh]) => {
              ...
              return (
                <div key={l} style={{ flex: 1, background: C.surface2, borderRadius: 20, padding: "14px 4px", textAlign: "center" }}>
                  <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 20, color: col, lineHeight: 1.1 }}>{v}<span style={{ fontSize: 11, fontWeight: 700, color: C.muted2 }}>/5</span></div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 24, marginTop: 16, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
            {[[athlete.days.filter(d => d === 1).length, t("Treningov"), C.accent], [athlete.days.filter(d => d === 0).length, t("Počitek"), C.muted]].map(([v, l, col]) => (
              <div key={l}>
                <span style={{ fontFamily: C.display, fontWeight: 800, fontSize: 22, color: col }}>{v}</span>
                <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.08em", marginLeft: 5 }}>{l}</Mono>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 28 }}>
          {lastTraining.map((ex, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 0", borderBottom: i < lastTraining.length - 1 ? `1px solid ${C.border}` : "none" }}>
              <Mono style={{ width: 18, fontSize: 10, color: C.muted2, flexShrink: 0 }}>{i + 1}</Mono>
              <span style={{ flex: 1, fontFamily: C.display, fontWeight: 600, fontSize: 15, color: C.text }}>{t(ex.name)}</span>
              <Mono style={{ color: C.muted, fontSize: 10 }}>{ex.sets}×{ex.reps}</Mono>
              <span style={{ fontFamily: C.display, fontWeight: 700, fontSize: 14.5, color: C.text2, marginLeft: 6, minWidth: 52, textAlign: "right" }}>{ex.load}</span>
            </div>
          ))}
        </div>
...
export default function ScreenClub({ go, profile }) {
  ...
  return (
    <div style={{ padding: "10px 18px 28px", color: C.text }}>
      <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase" }}>ATHLETE OS</Mono>
      <h1 style={{ fontFamily: C.display, fontWeight: 800, fontSize: 30, margin: "8px 0 24px", letterSpacing: "-0.02em" }}>{t("Klub")}</h1>

      {/* hero — club identity + team readiness summary */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
          <span style={{ width: 40, height: 40, borderRadius: 12, background: C.surface2, display: "flex", alignItems: "center", justifyContent: "center", color: C.text2, flexShrink: 0 }}><IcTrophy size={18} /></span>
          <div>
            <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 18, letterSpacing: "-0.01em" }}>{club}</div>
            <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.08em" }}>U17 · {TEAM.length} {t("članov")}</Mono>
          </div>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.16em", textTransform: "uppercase" }}>{t("READINESS · BATERIJA")}</Mono>
          <Mono style={{ color: avgCol, fontSize: 10, letterSpacing: "0.14em" }}>{avgBattery >= 70 ? "READY" : avgBattery >= 40 ? "CAUTION" : "REST"}</Mono>
        </div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          <div style={{ flex: 1, background: C.surface2, borderRadius: 20, padding: "13px 14px" }}>
            <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 20, color: C.text, lineHeight: 1.1 }}>{totalSessions}</div>
            <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", marginTop: 4, display: "block" }}>{t("Treningov")}</Mono>
          </div>
          <div style={{ flex: 1, background: C.surface2, borderRadius: 20, padding: "13px 14px" }}>
            <div style={{ fontFamily: C.display, fontWeight: 800, fontSize: 20, color: injuredCount > 0 ? C.red : C.muted2, lineHeight: 1.1 }}>{injuredCount}</div>
            <span style={{ display: "flex", color: injuredCount > 0 ? C.red : C.muted2, marginTop: 6 }}><IcBandage size={12} /></span>
          </div>
          <div style={{ flex: 1, background: C.surface2, borderRadius: 20, padding: "13px 14px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4, fontFamily: C.display, fontWeight: 800, fontSize: 20, color: topStreak > 0 ? C.text : C.muted2, lineHeight: 1.1 }}>
              <span style={{ display: "flex", color: topStreak > 0 ? C.gold : C.muted2 }}><IcFlame size={13} /></span>
              {topStreak}
            </div>
            <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.16em", marginTop: 4, display: "block" }}>STREAK</Mono>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "4px 2px 20px", marginBottom: 20 }}>
        <span style={{ width: 36, height: 36, borderRadius: "50%", background: C.surface2, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: C.display, fontWeight: 700, fontSize: 13, color: C.text2, flexShrink: 0 }}>M</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: C.display, fontWeight: 700, fontSize: 15 }}>Coach Matej</div>
          <Mono style={{ color: C.muted, fontSize: 10, letterSpacing: "0.06em" }}>{t("Glavni trener")}</Mono>
        </div>
      </div>
          ...
          return (
            <Card key={i} onClick={() => setDetail(member)} pad={14}>
              <span style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <span style={{ width: 40, height: 40, borderRadius: "50%", background: C.surface3, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: C.display, fontWeight: 700, fontSize: 13, color: C.text2, flexShrink: 0 }}>{member.ini}</span>
                <span style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ display: "block", fontFamily: C.display, fontWeight: 600, fontSize: 15, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{member.name}</span>
                  <span style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
                    <Mono style={{ color: C.muted, fontSize: 9, letterSpacing: "0.06em" }}>{t(member.pos)}</Mono>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                      <span style={{ display: "flex", color: member.streak > 0 ? C.gold : C.muted2 }}><IcFlame size={10} /></span>
                      <Mono style={{ color: member.streak > 0 ? C.muted : C.muted2, fontSize: 9, letterSpacing: "0.06em" }}>{member.streak}</Mono>
                    </span>
                ...
                <span style={{ textAlign: "right", minWidth: 44, flexShrink: 0 }}>
                  <span style={{ display: "block", fontFamily: C.display, fontWeight: 800, fontSize: 24, color: batCol, letterSpacing: "-0.02em", lineHeight: 1 }}>{member.battery}</span>
